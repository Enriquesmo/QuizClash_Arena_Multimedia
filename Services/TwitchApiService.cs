...
public class TwitchApiService
{
    ...
    {
        // URL del endpoint de Twitch para obtener la clave de transmisión
        ...
        {
            var errorContent = await response.Content.ReadAsStringAsync();
            throw new Exception($"Error al obtener la clave de transmisión: {response.StatusCode} - {errorContent}");
        }

        // Leer la respuesta JSON
        var responseContent = await response.Content.ReadAsStringAsync();

        // Deserializar la respuesta JSON para obtener la clave de transmisión
        var jsonResponse = JsonSerializer.Deserialize<JsonElement>(responseContent);
